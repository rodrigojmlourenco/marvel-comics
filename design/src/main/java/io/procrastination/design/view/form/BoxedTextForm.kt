 ...
 */
class BoxedTextForm : FrameLayout {
    ...
    init {
        LayoutInflater
            .from(context)
            .inflate(R.layout.view_boxed_text_form, this, true)
    ...
    private fun handleAttributes(attrs: AttributeSet?) {
        ...
        context.handleAttributeSet(attrs, R.styleable.BoxedTextForm) { array, i ->
            when (val id = array.getIndex(i)) {

                R.styleable.BoxedTextForm_android_hint -> {
                    editText.hint = array.getString(id)
                }

                R.styleable.BoxedTextForm_android_inputType -> {
                    setInputType(array.getInt(id, InputType.TYPE_CLASS_TEXT))
                }

                R.styleable.BoxedTextForm_android_imeOptions -> {
